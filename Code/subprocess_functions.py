...
def ffmpeg_conversion(audio_files: list, output_name: str):
    ...
    for i in audio_files:
        if not i[-3:] in acceptable_files:
            raise ValueError(
                f'The only acceptable file types in this program are: {acceptable_files}')
    ...
    with open("mylist.txt", "w") as file:
        for i in audio_files:
            if i[-3:] == container:
                ...
                raise ValueError(
                    "You selected files of multiple types. Please keep your selection to files of the same type.")
