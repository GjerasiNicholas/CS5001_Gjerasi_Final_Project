...
with open("Audio/mylist.txt","w") as file:
    for i in file_list:
        full_path = os.path.abspath(i) #Gets the absolute path. Relative paths are tricky and annoying tbh.
...
subprocess.run([
    ...
    "ffmpeg","-y", "-f","concat","-safe","0","-i","Audio/mylist.txt","-c","copy","Audio/concatOutput.m4b"
...
subprocess.run([
    ...
    "-c:a", "libmp3lame",
    "-ab", '190k',
    "Audio/outputfrompython.mp3"
]) 
# The "-y" means that ffmpeg will always overwrite instead of prompting me to do so.
# We use just "ffmpeg" not "C:/ffmpeg/bin/ffmpeg.exe" because we set the PATH variable for ffmpeg so we dont need the directory exactly.
